...
- 一次只能执行一个工具，无法表达同一 ExecutionPlan 内的并行工具调用。
- 工具选择、参数生成和工具执行没有独立的持久化边界。
- 进程崩溃或任务重新调度后，LLM 可能重复生成已经生成过的内容。
- 当前代码已经通过 `ToolSuccess.state_delta` 结构化描述工具对 `State` 的建议变更，但尚未实现统一应用。
...
3. 一个 ExecutionPlan 可以包含多个相互独立、可以并行执行的 ToolCall。
4. 执行 Phase 可以在 Rust 异步任务中 `await` 当前 ExecutionPlan 的工具执行，不要求完全采用 Netty 式回调模型。
5. 每个 ToolCall 完成后立即持久化结果和 `StateDelta`，但不直接修改共享 `State`。
6. ExecutionPlan 中的 ToolCall 全部到达终态后，Runtime 统一合并结果并一次性更新 `State`。
7. ExecutionPlan 部分成功时，保留成功 ToolCall 的结果和状态变化，同时记录失败事实。
8. ToolCall 是否重试以及可以重试多少次，由工具的重试策略决定。
9. ToolCall 重试耗尽意味着 ExecutionPlan 执行失败，不直接意味着整个 Task 失败；下一次由 LLM 决定如何继续。
...
- 支持同一 ExecutionPlan 内多个 ToolCall 有界并行执行。
...
- 不实现完整的人工交互界面和审批工作流。
- 不在第一版实现任意依赖图调度；ExecutionPlan 内只允许无相互依赖的 ToolCall。
- 不要求第一版支持动态扩缩容和跨区域调度。
...
| ExecutionPlan | 目标 Runtime 校验后、由一组可并行 ToolCall 组成的执行计划。 |
| ToolCall | 工具名称、持久化参数、执行状态、结果和幂等信息组成的一次逻辑调用。 |
| State | Runtime 对当前任务已知事实、业务知识、执行历史和进度的持久化表示。 |
| StateDelta | ToolCall 成功后建议对 State 进行的结构化变更；当前代码已定义类型，但尚未由统一 Reducer 应用。 |
...
| [工具执行](tool-execution.md) | ToolCall、ExecutionPlan 并行、参数、重试、幂等、StateDelta 和人工参与。 |
...
8. ExecutionPlan 完成后统一、确定性地合并 StateDelta。
9. ExecutionPlan 部分失败时不丢弃已经成功的事实和状态变化。
10. ExecutionPlan 失败不自动等同于 Task 失败。
